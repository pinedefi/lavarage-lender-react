...
interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: 'default' | 'primary' | 'secondary' | 'ghost' | 'link' | 'outline' | 'glass' | 'lavarage';
  size?: 'sm' | 'md' | 'lg';
  isLoading?: boolean;
  fullWidth?: boolean;
  asChild?: boolean;
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = 'default', size = 'md', isLoading, fullWidth, asChild = false, children, ...props }, ref) => {
    ...
    const variants = {
      ...
      outline: 'border border-lavarage-orange/30 text-lavarage-red hover:bg-lavarage-subtle hover:border-lavarage-coral',
      glass: 'btn-glass',
    ...
    return (
      ...
      >
        {isLoading ? (
          <span className="loading-lavarage mr-2 h-4 w-4" />
        ) : null}
        {children}
      </Comp>
    );
